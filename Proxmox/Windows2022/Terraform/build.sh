...
if [ ! -f "secrets.auto.tfvars" ]; then
  echo "Error: secrets.auto.tfvars file not found."
  echo "Please copy secrets.auto.tfvars.example to secrets.auto.tfvars and edit it."
  exit 1
fi
...
VAR_FILES="-var-file=terraform.tfvars -var-file=secrets.auto.tfvars"

# Handle different execution modes
if [ "$DESTROY" = true ]; then
  echo "Destroying Windows Server 2022 infrastructure..."
  terraform destroy $VAR_FILES
elif [ "$PLAN_ONLY" = true ]; then
  echo "Creating plan only..."
  terraform plan $VAR_FILES -out=tfplan
else
  echo "Creating execution plan for Windows Server 2022 deployment..."
  terraform plan $VAR_FILES -out=tfplan
  
  echo "Applying execution plan..."
  terraform apply tfplan
  
  echo "Windows Server 2022 deployment complete!"
  echo ""
